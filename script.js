...
const codeOutput2 = document.getElementById('code-output2');
const codeLines = [
...
const typingSpeed2 = 1; 
codeOutput2.style.color = '#4B0082';
...
function typeCode2() {
    const interval = setInterval(() => {
        if (lineIndex >= codeLines2.length) {
            clearInterval(interval);
            return;
        }

        const currentLine = codeLines2[lineIndex];
        if (charIndex < currentLine.length) {
            codeOutput2.textContent += currentLine.slice(charIndex, charIndex + charsPerInterval);
            charIndex += charsPerInterval;
        } else {
            codeOutput2.textContent += '\n';
            lineIndex++;
            charIndex = 0;
        }
    }, typingSpeed2);
